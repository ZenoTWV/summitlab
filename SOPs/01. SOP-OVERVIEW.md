...
Use @SONNET-BRAINSTORM-PROMPT.md

**Answer all questions in one message.**

### Step 2: Plan the Project (Claude Opus in Plan Mode)
This is a natural next step and will already be done after step 1.
When exiting plan more opusplan, sonnet will create a github issue in the right repo and then start building the initial state of the project.
WHen this is done building, Sonnet will tag the initial version.

### Step 3: Build Features (Claude Sonnet)
**Normal workflow from here:**
- Create issues for each feature from opusplan_project_brief.md
- Work on issues one by one
- Follow regular workflow (issue → DoD → build → commit → merge)

### Step 4: Launch (Tag v1.0.0)
